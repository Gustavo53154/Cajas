...
const App = () => {
  ...
  const [tasks, setTasks] = useState(["Coches", "Devoluciones", "Bolsas", "Cajas de Agua", "Limpieza de Cajas"]);
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      const filtered = data.filter(({ apellidos, nombres, horarios }) => {
        const alreadyAssigned = Object.values(assignments).some(
          (assignment) => assignment.nombre === `${apellidos}, ${nombres}`
        );
  ...
  const toggleStatus = (caja) => {
    setAssignments((prev) => ({
      ...prev,
      [caja]: { ...prev[caja]?.active ? { ...prev[caja], active: !prev[caja].active } : {} },
    }));
  };
  ...
  return (
    <div className="p-4 flex flex-wrap justify-between gap-8">
      {/* NO ASIGNADOS */}
      <div className="w-full sm:w-1/3 max-w-3xl">
        <h1 className="text-center text-xl font-bold mb-4">NO ASIGNADOS</h1>
        <table className="table-auto w-full border border-gray-300 mb-6 text-center">
          <thead>
            ...
            {tasks.map((task, index) => (
              <React.Fragment key={index}>
                ...
                {taskAssignments.filter(t => t.task === task).map((t, i) => (
                  <tr key={i}>
                    <td></td>
                    <td>({t.caja}) {t.nombre}  {t.horario}</td>
                    <td>
